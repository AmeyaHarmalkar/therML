...
def train_singer(
    ...
    data = {
        mode: SingerDataset(mode, seed=args.seed, feature_strategy=feature_strategy)
        for mode in Mode
    }
...
def train(args, train_args):
    ...
    data = {
        mode: (PairedStabilityDataset if args.paired else StabilityDataset)(
            split=args.split,
            mode=mode,
            seed=args.seed,
...
def eval_unsupervised(checkpoint: str, outfile: str):
    from model import ThermophileClassifier
    from tqdm import tqdm
    import pickle as pkl

    data = [PairedStabilityDataset(mode="test", split=split) for split in range(17)]
    model = ThermophileClassifier.load_from_checkpoint(checkpoint)
    model = model.eval().cuda().requires_grad_(False)
    results = {}
    for split in data:
        loader = DataLoader(
            split, num_workers=3, collate_fn=split.collater, batch_size=32
        )
        predictions = []
        diffs = []
        for b1, b2, _ in tqdm(loader):
            prediction = model(b1["tokens"].cuda(), b2["tokens"].cuda())
            predictions.append(prediction.cpu().numpy())
            diffs.append((b1["ts50"] - b2["ts50"]).cpu().numpy())
        results[split.split] = (
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--split", default="0", choices=list(map(str, range(17))) + ["all"]
    )
    parser.add_argument(
        "--base_model",
        type=str,
